...
public interface RoleMapper {
    //配置下划线转驼峰@Select({"select * from sys_role where id = #{id}"})或@Select({"select id,role_name,enabled,create_by,create_time,from sys_role where id = #{id}"})
    ...
    @Update({"update sys_role set role_name = #{roleName},enabled=#{enabled},create_by=#{createInfo.createBy},create_time=#{createInfo.createTime,jdbcType=TIMESTAMP} where id=#{id}"})
